...
use egui_plot::{Line, Plot, PlotPoint, PlotPoints, Points, Polygon};
...
impl FanCurveEditor {
    ...
    fn draw_points_editor(&mut self, ui: &mut Ui) {
        ...
        egui::Grid::new(format!("points_grid_{}", self.fan_id))
            ...
            .show(ui, |ui| {
                ...
                for (idx, (temp, speed)) in self.curve.points.iter().enumerate() {
                    ...
                    if ui
                        .add(egui::Slider::new(&mut temp_val, 0.0..=100.0).suffix("°C"))
                        .changed()
                    {
                    ...
                    if ui
                        .add(egui::Slider::new(&mut speed_val, 0.0..=100.0).suffix("%"))
                        .changed()
                    {
        ...
        if self.selected_point.is_some() {
            ui.label(
                RichText::new("💡 Tip: Click and drag points on the graph to adjust them")
                    .small()
                    .italics(),
            );
        }
    ...
    fn reset_to_default(&mut self) {
        self.curve.points = vec![(0, 0), (50, 50), (70, 75), (85, 100)];
        self.selected_point = None;
        self.dragging_point = None;
    }
