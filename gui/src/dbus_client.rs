...
pub enum DbusCommand {
    ...
    GetMemoryInfo {
        reply: oneshot::Sender<Result<MemoryInfo>>,
    },
    GetCpuInfo {
...
impl DbusClient {
    ...
    pub fn get_memory_info(&self) -> oneshot::Receiver<Result<MemoryInfo>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::GetMemoryInfo { reply: tx });
        rx
    }
    ...
    pub fn get_tdp_profiles(&self) -> oneshot::Receiver<Result<Vec<String>>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::GetTdpProfiles { reply: tx });
        rx
    }
    ...
    pub fn get_hardware_interface_info(&self) -> oneshot::Receiver<Result<String>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::GetHardwareInterfaceInfo { reply: tx });
        rx
    }
    ...
    pub fn set_amd_pstate_status(&self, status: String) -> oneshot::Receiver<Result<()>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::SetAmdPstateStatus { status, reply: tx });
        rx
    }
    ...
    pub fn set_fan_auto(&self, fan_id: u32) -> oneshot::Receiver<Result<()>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::SetFanAuto { fan_id, reply: tx });
        rx
    }
    ...
    pub fn set_gpu_locked_clocks(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::SetGpuLockedClocks {
            device_index,
            min_clock,
            max_clock,
            reply: tx,
    ...
    pub fn reset_gpu_clocks(&self, device_index: u32) -> oneshot::Receiver<Result<()>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::ResetGpuClocks {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn get_gpu_clock_ranges(&self, device_index: u32) -> oneshot::Receiver<Result<(u32, u32)>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::GetGpuClockRanges {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn get_gpu_mem_clock_ranges(
        &self,
        device_index: u32,
    ) -> oneshot::Receiver<Result<Vec<u32>>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::GetGpuMemClockRanges {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn set_memory_locked_clocks(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::SetMemoryLockedClocks {
            device_index,
            min_clock,
            max_clock,
            reply: tx,
    ...
    pub fn reset_memory_locked_clocks(&self, device_index: u32) -> oneshot::Receiver<Result<()>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::ResetMemoryLockedClocks {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn set_gpu_core_offset(
        &self,
        device_index: u32,
        offset: i32,
    ) -> oneshot::Receiver<Result<()>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::SetGpuCoreOffset {
            device_index,
            offset,
            reply: tx,
        });
    ...
    pub fn set_gpu_memory_offset(
        &self,
        device_index: u32,
        offset: i32,
    ) -> oneshot::Receiver<Result<()>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::SetGpuMemoryOffset {
            device_index,
            offset,
            reply: tx,
        });
    ...
    pub fn get_gpu_core_offset_limits(
        &self,
        device_index: u32,
    ) -> oneshot::Receiver<Result<(i32, i32)>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::GetGpuCoreOffsetLimits {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn get_gpu_memory_offset_limits(
        &self,
        device_index: u32,
    ) -> oneshot::Receiver<Result<(i32, i32)>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::GetGpuMemoryOffsetLimits {
            device_index,
            reply: tx,
        });
        rx
    ...
    pub fn set_prime_profile(&self, profile: &str) -> oneshot::Receiver<Result<()>> {
        let (tx, rx) = oneshot::channel();
        let _ = self.command_tx.send(DbusCommand::SetPrimeProfile {
    ...
    pub fn shutdown_daemon(&self) -> oneshot::Receiver<Result<()>> {
        ...
        let _ = self
            .command_tx
            .send(DbusCommand::ShutdownDaemon { reply: tx });
        rx
    }
}
...
async fn dbus_worker(mut command_rx: mpsc::UnboundedReceiver<DbusCommand>) -> Result<()> {
    ...
    while let Some(command) = command_rx.recv().await {
        match command {
            ...
            DbusCommand::SetGpuLockedClocks {
                device_index,
                min_clock,
                max_clock,
                reply,
                ...
                let result =
                    set_gpu_locked_clocks_impl(&connection, device_index, min_clock, max_clock)
                        .await;
                let _ = reply.send(result);
            }
            DbusCommand::ResetGpuClocks {
                device_index,
                reply,
            } => {
            ...
            DbusCommand::GetGpuMemClockRanges {
                device_index,
                reply,
            } => {
            ...
            DbusCommand::SetMemoryLockedClocks {
                device_index,
                min_clock,
                max_clock,
                reply,
                ...
                let result =
                    set_memory_locked_clocks_impl(&connection, device_index, min_clock, max_clock)
                        .await;
                let _ = reply.send(result);
            }
            DbusCommand::ResetMemoryLockedClocks {
                device_index,
                reply,
            } => {
            ...
            DbusCommand::SetGpuMemoryOffset {
                device_index,
                offset,
                reply,
            } => {
            ...
            DbusCommand::GetGpuCoreOffsetLimits {
                device_index,
                reply,
            } => {
            ...
            DbusCommand::GetGpuMemoryOffsetLimits {
                device_index,
                reply,
            } => {
...
async fn set_amd_pstate_status_impl(conn: &Connection, status: &str) -> Result<()> {
    ...
    proxy
        .call::<_, _, ()>("SetAmdPstateStatus", &(status,))
        .await?;
    Ok(())
}
...
async fn set_gpu_locked_clocks_impl(
    conn: &Connection,
    device_index: u32,
    min_clock: u32,
    max_clock: u32,
    ...
    .await?;
    proxy
        .call::<_, _, ()>("SetGpuLockedClocks", &(device_index, min_clock, max_clock))
        .await?;
    Ok(())
}
...
async fn reset_gpu_clocks_impl(conn: &Connection, device_index: u32) -> Result<()> {
    ...
    .await?;
    proxy
        .call::<_, _, ()>("ResetGpuClocks", &(device_index,))
        .await?;
    Ok(())
}
...
async fn set_memory_locked_clocks_impl(
    conn: &Connection,
    device_index: u32,
    min_clock: u32,
    max_clock: u32,
    ...
    proxy
        .call::<_, _, ()>(
            "SetMemoryLockedClocks",
            &(device_index, min_clock, max_clock),
        )
        .await?;
    Ok(())
}
...
async fn reset_memory_locked_clocks_impl(conn: &Connection, device_index: u32) -> Result<()> {
    ...
    .await?;
    proxy
        .call::<_, _, ()>("ResetMemoryLockedClocks", &(device_index,))
        .await?;
    Ok(())
}
...
async fn set_gpu_core_offset_impl(conn: &Connection, device_index: u32, offset: i32) -> Result<()> {
    ...
    .await?;
    proxy
        .call::<_, _, ()>("SetGpuCoreOffset", &(device_index, offset))
        .await?;
    Ok(())
}

async fn set_gpu_memory_offset_impl(
    conn: &Connection,
    device_index: u32,
    offset: i32,
) -> Result<()> {
    ...
    .await?;
    proxy
        .call::<_, _, ()>("SetGpuMemoryOffset", &(device_index, offset))
        .await?;
    Ok(())
}

async fn get_gpu_core_offset_limits_impl(
    conn: &Connection,
    device_index: u32,
) -> Result<(i32, i32)> {
    let proxy = zbus::Proxy::new(
    ...
    .await?;
    let limits: (i32, i32) = proxy
        .call("GetGpuCoreOffsetLimits", &(device_index,))
        .await?;
    Ok(limits)
}

async fn get_gpu_memory_offset_limits_impl(
    conn: &Connection,
    device_index: u32,
) -> Result<(i32, i32)> {
    let proxy = zbus::Proxy::new(
    ...
    .await?;
    let limits: (i32, i32) = proxy
        .call("GetGpuMemoryOffsetLimits", &(device_index,))
        .await?;
    Ok(limits)
}
...
async fn set_prime_profile_impl(conn: &Connection, profile: &str) -> Result<()> {
    ...
    .await?;
    proxy
        .call::<_, _, ()>("SetPrimeProfile", &(profile,))
        .await?;
    Ok(())
}
