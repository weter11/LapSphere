...
use egui::{ComboBox, RichText, ScrollArea, Slider, TopBottomPanel, Ui};
use std::path::Path;
...
fn draw_cpu_tuning(
    ...
    if !tdp_profiles.is_empty() {
        ...
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("performance_profile_combo")
                ...
                .show_ui(ui, |ui| {
                    if ui
                        .selectable_label(current_profile.is_none(), "System default")
                        .clicked()
                    {
    ...
    if caps.has_scaling_min_freq && caps.has_scaling_max_freq {
        ...
        if let (Some(hw_min), Some(hw_max)) = (cpu_info.hw_min_freq, cpu_info.hw_max_freq) {
            let mut min_freq = profile.cpu_settings.min_frequency.unwrap_or(hw_min) as f64 / 1000.0;
            let mut max_freq = profile.cpu_settings.max_frequency.unwrap_or(hw_max) as f64 / 1000.0;
...
fn draw_gpu_tuning(
    ...
    ui.checkbox(
        &mut profile.gpu_settings.manual_clocks,
        "Enable Manual Clock Control",
    );
    ...
    if profile.gpu_settings.manual_clocks {
        ...
        if state.gpu_mem_offset_limits.is_none() {
            if let Some(client) = dbus_client {
                ...
                tokio::spawn(async move {
                    let res = client
                        .get_gpu_memory_offset_limits(0)
                        .await
                        .map(|r| r.map_err(|e| e.to_string()))
                        .unwrap_or_else(|e| Err(e.to_string()));
...
fn apply_gpu_settings_on_save(
    client: &DbusClient,
    gpu_settings: &tuxedo_common::types::GpuSettings,
) {
    if gpu_settings.manual_clocks {
        // Apply GPU locked clocks if set
        if let (Some(min_clock), Some(max_clock)) =
            (gpu_settings.min_gpu_clock, gpu_settings.max_gpu_clock)
        {
        ...
        if let (Some(min_mem), Some(max_mem)) =
            (gpu_settings.min_mem_clock, gpu_settings.max_mem_clock)
        {
...
fn draw_keyboard_tuning(ui: &mut Ui, profile: &mut Profile, dbus_client: Option<&DbusClient>) {
    ...
    if profile.keyboard_settings.control_enabled {
        ...
        } else if !keyboard_caps.supports_effects {
            ui.label(
                RichText::new("Effect support was not detected; some modes may not apply.").small(),
            );
        }
        ...
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("keyboard_mode")
                ...
                .show_ui(ui, |ui| {
                    ...
                    if ui
                        .selectable_label(current_mode_name == "Dance", "Dance")
                        .clicked()
                    {
                        profile.keyboard_settings.mode = KeyboardMode::Dance {
                            brightness: 50,
                            speed: 50,
                        };
                        preview_needed = true;
                    ...
                    if ui
                        .selectable_label(current_mode_name == "Flash", "Flash")
                        .clicked()
                    {
                        profile.keyboard_settings.mode = KeyboardMode::Flash {
                            r: 255,
                            g: 255,
                            b: 255,
                            brightness: 50,
                    ...
                    if ui
                        .selectable_label(current_mode_name == "Random Color", "Random Color")
                        .clicked()
                    {
                        profile.keyboard_settings.mode = KeyboardMode::RandomColor {
                            brightness: 50,
                            speed: 50,
                        };
                        preview_needed = true;
                    ...
                    if ui
                        .selectable_label(current_mode_name == "Tempo", "Tempo")
                        .clicked()
                    {
                        profile.keyboard_settings.mode = KeyboardMode::Tempo {
                            brightness: 50,
                            speed: 50,
                        };
                        preview_needed = true;
        ...
        match &mut profile.keyboard_settings.mode {
            KeyboardMode::SingleColor {
                ...
                ui.horizontal(|ui| {
                    ui.label("Brightness:");
                    if ui
                        .add(Slider::new(brightness, 0..=100).suffix("%"))
                        .changed()
                    {
                        preview_needed = true;
                    }
