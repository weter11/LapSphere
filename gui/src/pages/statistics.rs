...
use egui::Color32;
use egui::{CollapsingHeader, Grid, ProgressBar, RichText, ScrollArea, Ui};
            ...
            if let Some(ref cpu) = state.cpu_info {
                Grid::new("cpu_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label(
                            RichText::new(format!("{} MHz", cpu.median_frequency / 1000))
                                .monospace(),
                        );
                        ui.end_row();
                        ...
                        if let Some(power) = cpu.package_power {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if let Some(ref source) = cpu.power_source {
                                    ui.label(
                                        RichText::new(format!("({})", source)).small().italics(),
                                    );
                                }
                            });
                            ui.end_row();
                            ...
                            ui.vertical(|ui| {
                                for source in &cpu.all_power_sources {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.label(
                                            RichText::new(format!("{:.1} W", source.value))
                                                .small()
                                                .monospace(),
                                        );
                                    });
                        ...
                        Grid::new("cores_grid")
                            ...
                            .show(ui, |ui| {
                                ...
                                for core in &cpu.cores {
                                    ...
                                    ui.label(
                                        RichText::new(format!("{} MHz", core.frequency / 1000))
                                            .monospace(),
                                    );
                                    ui.add(
                ...
                for (idx, gpu) in state.gpu_info.iter().enumerate() {
                    ...
                    Grid::new(format!("gpu_grid_{}", idx))
                        ...
                        .show(ui, |ui| {
                            ui.label("Type:");
                            ui.label(
                                if gpu.gpu_type == tuxedo_common::types::GpuType::Integrated {
                                    "Integrated"
                                } else {
                                    "Discrete"
                                },
                            ...
                            if let Some(temp) = gpu.temperature {
                                ...
                                ui.colored_label(temp_color(temp), format!("{:.1}°C", temp));
                                ui.end_row();
                            }
                            ...
                            if let Some(load) = gpu.load {
                                ui.label("Load:");
                                ui.add(
                                    ProgressBar::new(load / 100.0).text(format!("{:.1}%", load)),
                                );
                                ui.end_row();
                            }
                            ...
                            if let Some(power) = gpu.power {
                                ui.label("Power:");
                                ui.colored_label(power_color(power), format!("{:.1} W", power));
                                ui.end_row();
                            }
                        });
                }
                    ...
                    .show(ui, |ui| {
                        ...
                        if power_w.abs() > 0.1 {
                            ...
                            ui.colored_label(
                                ...
                                format!("{:.1} W {}", power_w.abs(), direction),
                            );
                            ui.end_row();
                        }
                    ...
                    Grid::new(format!("wifi_grid_{}", wifi.interface))
                        ...
                        .show(ui, |ui| {
                            ...
                            if let Some(signal) = wifi.signal_level {
                                ...
                                ui.horizontal(|ui| {
                                    let signal_percent =
                                        ((signal + 90) as f32 / 60.0).clamp(0.0, 1.0);
                            ...
                            if let Some(tx_rate) = wifi.tx_rate {
                                ui.label("TX Rate:");
                                ui.label(RichText::new(format!("{:.1} Mbps", tx_rate)).monospace());
                                ui.end_row();
                            }

                            if let Some(rx_rate) = wifi.rx_rate {
                                ui.label("RX Rate:");
                                ui.label(RichText::new(format!("{:.1} Mbps", rx_rate)).monospace());
                                ui.end_row();
                            }
