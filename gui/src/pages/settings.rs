...
use egui::{ComboBox, Context, RichText, ScrollArea, Slider, Ui};
...
pub fn draw(ui: &mut Ui, state: &mut AppState, theme: &mut TuxedoTheme, ctx: &Context) {
    ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            let mut battery_poll =
                (state.config.statistics_sections.battery_poll_rate as f32) / 1000.0;
            ui.horizontal(|ui| {
                ui.label("Battery:");
                if ui
                    .add(
                    ...
                    .changed()
                {
                    state.config.statistics_sections.battery_poll_rate =
                        (battery_poll * 1000.0) as u64;
                    let _ = state.save_config();
                }
            });
            ...
            let mut storage_poll =
                (state.config.statistics_sections.storage_poll_rate as f32) / 1000.0;
            ui.horizontal(|ui| {
                ui.label("Storage:");
                if ui
                    .add(
                        Slider::new(&mut storage_poll, 5.0..=60.0)
                            .step_by(0.5)
                            .suffix(" s"),
                    )
                    .changed()
                {
                    state.config.statistics_sections.storage_poll_rate =
                        (storage_poll * 1000.0) as u64;
                    let _ = state.save_config();
                }
            });
