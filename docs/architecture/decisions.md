...
**Reason:** This centralizes periodic work and lets DBus update intervals
without directly mutating the job heap. `[verified]`

**Consequences:** Job callbacks execute in the scheduler task, while DBus
...
**Reason:** Polling callbacks and DBus methods need the same state. `[verified]`
...
NVML query; other DBus hardware methods call synchronous functions directly.
`[verified]`
