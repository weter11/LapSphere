...
pub fn apply_profile(profile: &Profile) -> Result<()> {
    ...
    // Apply NVIDIA GPU settings (best effort)
    apply_nvidia_gpu_settings(&profile.gpu_settings)?;
...
fn apply_nvidia_gpu_settings(settings: &GpuSettings) -> Result<()> {
    let has_gpu_clocks =
        settings.nvidia_gpu_min_clock.is_some() && settings.nvidia_gpu_max_clock.is_some();
    let has_mem_clocks =
    ...
    if !(has_gpu_clocks
        ...
        || settings.nvidia_mem_offset.is_some())
    {
        return Ok(());
    }
    ...
    if let Ok(nvml) = Nvml::init() {
        if let Ok(mut device) = nvml.device_by_index(0) {
            if has_gpu_clocks {
                if let (Some(min), Some(max)) =
                    (settings.nvidia_gpu_min_clock, settings.nvidia_gpu_max_clock)
                {
            ...
            if has_mem_clocks {
                if let (Some(min), Some(max)) =
                    (settings.nvidia_mem_min_clock, settings.nvidia_mem_max_clock)
                {
                    let _ = device.set_mem_locked_clocks(min, max);
                }
            }

            if let Some(offset) = settings.nvidia_core_offset {
                let _ = device.set_clock_offset(Clock::Graphics, PerformanceState::Zero, offset);
            }

            if let Some(offset) = settings.nvidia_mem_offset {
                let _ = device.set_clock_offset(Clock::Memory, PerformanceState::Zero, offset);
            }
    ...
    // Fallback to nvidia-smi / nvidia-settings
    if has_gpu_clocks {
        if let (Some(min), Some(max)) =
            (settings.nvidia_gpu_min_clock, settings.nvidia_gpu_max_clock)
        {
            let _ = Command::new("nvidia-smi")
                .args(["-lgc", &format!("{},{}", min, max)])
                .output();
        }
    }
...
fn apply_keyboard_settings(settings: &KeyboardSettings) -> Result<()> {
    ...
    let base_path =
        find_keyboard_backlight_path().ok_or_else(|| anyhow!("Keyboard backlight not found"))?;
    ...
    match &settings.mode {
        KeyboardMode::SingleColor {
            ...
            log::info!(
                "Applying keyboard: RGB({}, {}, {}) brightness {}%",
                r,
                g,
                b,
                brightness
...
pub fn set_energy_performance_preference(epp: &str) -> Result<()> {
    ...
    for i in 0..cpu_count {
        ...
        if Path::new(&path).exists() {
            fs::write(&path, epp).map_err(|e| anyhow!("Failed to set EPP for CPU {}: {}", i, e))?;
        }
    }
...
impl RgbKeyboardControl {
    ...
    pub fn get_brightness(&self) -> Result<u8> {
        ...
        let current: u32 = fs::read_to_string(&brightness_path)?.trim().parse()?;
