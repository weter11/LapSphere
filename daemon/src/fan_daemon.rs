...
impl FanCurveManager {
    ...
    pub fn apply_curves(&mut self) -> Result<()> {
        ...
        let io = self
            .io
            .as_ref()
            .ok_or_else(|| anyhow!("TuxedoIo not available"))?;
        let settings = self
            .settings
            .as_ref()
            .ok_or_else(|| anyhow!("No settings configured"))?;
...
impl ControlInterface {
    async fn set_fan_curve(&self, fan_id: u32, curve_json: &str) -> Result<(), zbus::fdo::Error> {
        ...
        if curve.points.is_empty() {
            return Err(zbus::fdo::Error::Failed(
                "Curve must have at least one point".to_string(),
            ));
        }

        if curve.points.len() > 16 {
            return Err(zbus::fdo::Error::Failed(
                "Curve can have at most 16 points".to_string(),
            ));
        }
        ...
        for (temp, speed) in &curve.points {
            if *temp > 100 {
                return Err(zbus::fdo::Error::Failed(
                    "Temperature must be 0-100°C".to_string(),
                ));
            }
            if *speed > 100 {
    ...
    async fn get_fan_curve(&self, fan_id: u32) -> Result<String, zbus::fdo::Error> {
        ...
        serde_json::to_string(&curve).map_err(|e| zbus::fdo::Error::Failed(e.to_string()))
    }
